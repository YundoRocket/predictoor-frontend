...
export const PredictoorsProvider: React.FC<TPredictoorsContextProps> = ({
  ...
  const authorizationDataInstance = useRef<AuthorizationData<TAuthorization> | undefined>(undefined)
  ...
  const initializeContracts = useCallback(
    async (
      ...
      if (!tempSigner || !isCorrectNetwork) {
        ...
        tempSigner = randomSigner as unknown as ethers.providers.JsonRpcSigner
      }
  ...
  function arraysAreSame(
    x: Array<{ address: string }>,
    y: Array<{ address: string }>
  ): boolean {
        ...
        getMultiplePredictions({
          ...
          subscribedPredictoorsNew.forEach((contract) => {
            ...
            const blockchainFeedData = {
              contractInfo: contracts?.[contract.address] ?? null,
              predictions: dataPredictions
            }

            // Type assertion: predictions contain all required fields from TSocketFeedItem
            handleEpochData([blockchainFeedData] as Parameters<typeof handleEpochData>[0])
          })
